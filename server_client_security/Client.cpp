			...
			if(strstr(buf, "PUT"))
			{
				...
				if(fgets(sizebuf, 1024, stdin))
				{
					for(char* pdigit = sizebuf; *pdigit != '\n'; ++pdigit)
					{
						if(!isdigit(*pdigit))
						{
							fprintf(stderr, "Size %d must be in decimal\n", *pdigit);
							continue;
						}
					}
				}
				...
				if(fgets(data, 1024, stdin))
				{
					...
					fprintf(stdout, "write was a success!\n");
					continue;
				}
			}
			else
	...
	while(1)
	{
		...
		if(fgets(buff, 1024, stdin))
		{
			...
			if(strstr(buff, "GET"))
			{
				...
				char thebuff[1024];
				if(!read(fd, thebuff, sizeof(thebuff)))
				{
					fprintf(stderr, "read error\n");
					exit(0);
				}
				FILE* fp = fopen(save_name, "rw+");
				if(!fp)
				{
